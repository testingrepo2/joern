...
// TODO: Move each test below to their corresponding test category file
class SimpleAstCreationPassTest extends RubyCode2CpgFixture {
  ...
  "AST generation for simple fragments" should {
    ...
    "have correct structure for a +integer, decimal literal" in {
      val cpg           = code("+1")
      val List(plusOne) = cpg.call(Operators.plus).l
      plusOne.methodFullName shouldBe Operators.plus
      plusOne.name shouldBe Operators.plus
      plusOne.code shouldBe "+1"

      val List(one) = plusOne.argument.isLiteral.l
      one.typeFullName shouldBe "__builtin.Integer"
      one.code shouldBe "1"
      one.lineNumber shouldBe Some(1)
      one.columnNumber shouldBe Some(1)
    }

    "have correct structure for a -integer, decimal literal" in {
      val cpg            = code("-1")
      val List(minusOne) = cpg.call(Operators.minus).l
      minusOne.methodFullName shouldBe Operators.minus
      minusOne.name shouldBe Operators.minus
      minusOne.code shouldBe "-1"

      val List(one) = minusOne.argument.isLiteral.l
      one.typeFullName shouldBe "__builtin.Integer"
      one.code shouldBe "1"
      one.lineNumber shouldBe Some(1)
      one.columnNumber shouldBe Some(1)
    ...
    "have correct structure for a +float, decimal literal" in {
      val cpg          = code("+3.14")
      val List(plusPi) = cpg.call(Operators.plus).l
      plusPi.methodFullName shouldBe Operators.plus
      plusPi.name shouldBe Operators.plus
      plusPi.code shouldBe "+3.14"

      val List(pi) = plusPi.argument.isLiteral.l
      pi.typeFullName shouldBe "__builtin.Float"
      pi.code shouldBe "3.14"
      pi.lineNumber shouldBe Some(1)
      pi.columnNumber shouldBe Some(1)
    }

    "have correct structure for a -float, decimal literal" in {
      val cpg           = code("-3.14")
      val List(minusPi) = cpg.call(Operators.minus).l
      minusPi.methodFullName shouldBe Operators.minus
      minusPi.name shouldBe Operators.minus
      minusPi.code shouldBe "-3.14"

      val List(pi) = minusPi.argument.isLiteral.l
      pi.typeFullName shouldBe "__builtin.Float"
      pi.code shouldBe "3.14"
      pi.lineNumber shouldBe Some(1)
      pi.columnNumber shouldBe Some(1)
    ...
    "have correct structure for a -integer, binary literal" in {
      ...
      minusOne.methodFullName shouldBe Operators.minus
      minusOne.name shouldBe Operators.minus
      minusOne.code shouldBe "-0b01"

      val List(one) = minusOne.argument.isLiteral.l
      one.typeFullName shouldBe "__builtin.Integer"
      one.code shouldBe "0b01"
      one.lineNumber shouldBe Some(1)
      one.columnNumber shouldBe Some(1)
    }
    ...
    "have correct structure for a -integer, hexadecimal literal" in {
      val cpg            = code("-0xa")
      val List(minusTen) = cpg.call(Operators.minus).l
      minusTen.methodFullName shouldBe Operators.minus
      minusTen.name shouldBe Operators.minus
      minusTen.code shouldBe "-0xa"

      val List(ten) = minusTen.argument.isLiteral.l
      ten.typeFullName shouldBe "__builtin.Integer"
      ten.code shouldBe "0xa"
      ten.lineNumber shouldBe Some(1)
      ten.columnNumber shouldBe Some(1)
    }

    "have correct structure for a +integer, hexadecimal literal" in {
      val cpg           = code("+0xa")
      val List(plusTen) = cpg.call(Operators.plus).l
      plusTen.methodFullName shouldBe Operators.plus
      plusTen.name shouldBe Operators.plus
      plusTen.code shouldBe "+0xa"

      val List(one) = plusTen.argument.isLiteral.l
      one.typeFullName shouldBe "__builtin.Integer"
      one.code shouldBe "0xa"
      one.lineNumber shouldBe Some(1)
      one.columnNumber shouldBe Some(1)
    }

    // TODO
    "have correct structure for `self` identifier" ignore {
      val cpg           = code("self")
    ...
    "have correct structure for `__FILE__` identifier" in {
      ...
      file.columnNumber shouldBe Some(0)
    }

    "have correct structure for `__LINE__` identifier" in {
      val cpg        = code("__LINE__")
    ...
    "have correct structure for a unary `~` expression" in {
      val cpg            = code("~y")
    ...
    "have correct structure for a bitwise `&` expression" in {
      val cpg            = code("x & y")
      val List(callNode) = cpg.call.name(Operators.and).l
      callNode.code shouldBe "x & y"
      callNode.lineNumber shouldBe Some(1)
      callNode.columnNumber shouldBe Some(0)
    }
    ...
    "have correct structure for a bitwise `|` expression" in {
      val cpg            = code("x | y")
      val List(callNode) = cpg.call.name(Operators.or).l
      callNode.code shouldBe "x | y"
      callNode.lineNumber shouldBe Some(1)
      callNode.columnNumber shouldBe Some(0)
    }
    ...
    "have correct structure for a bitwise `^` expression" in {
      val cpg            = code("x ^ y")
      val List(callNode) = cpg.call.name(Operators.xor).l
      callNode.code shouldBe "x ^ y"
      callNode.lineNumber shouldBe Some(1)
      callNode.columnNumber shouldBe Some(0)
    }
    ...
    "have correct structure for empty class declaration" in {
      val cpg = code("""
          |class MyClass
          |end
          |""".stripMargin)

      val List(typeDecl) = cpg.typeDecl("MyClass").l
      typeDecl.isExternal shouldBe false
      typeDecl.inheritsFromTypeFullName shouldBe Seq()
      typeDecl.name shouldBe "MyClass"
      typeDecl.fullName shouldBe "Test0.rb:<global>::program.MyClass"
      typeDecl.lineNumber shouldBe Some(2)
    }

    "have correct structure for empty class declaration extending `MyBaseClass`" in {
      val cpg = code("""
      ...
      val List(typeDecl) = cpg.typeDecl("MyClass").l
      typeDecl.isExternal shouldBe false
      typeDecl.inheritsFromTypeFullName shouldBe Seq("MyBaseClass") // TODO: MyBaseClass isn't a full name
      typeDecl.name shouldBe "MyClass"
      typeDecl.fullName shouldBe "Test0.rb:<global>::program.MyClass"
      typeDecl.lineNumber shouldBe Some(2)
    }
    ...
    "have correct structure for `until` statement" in {
      val cpg = code("""
          |until i == 6 do
          |  i += 1
          |end
          |""".stripMargin)
      val List(until) = cpg.controlStructure.l
      until.controlStructureType shouldBe ControlStructureTypes.WHILE

      val List(notCondition: Call, body) = until.astChildren.l: @unchecked
      notCondition.methodFullName shouldBe Operators.logicalNot
      notCondition.name shouldBe Operators.logicalNot
      notCondition.code shouldBe "i == 6"

      val List(condition: Call) = notCondition.argument.l: @unchecked
      condition.methodFullName shouldBe Operators.equals
      condition.name shouldBe Operators.equals
      condition.code shouldBe "i == 6"

      // TODO: Test body
    }

    "have correct structure for `while` statement" in {
      val cpg = code("""
          |while i < 6 do
          |  i += 1
          |end
      ...
      val List(whileNode) = cpg.controlStructure.l
      whileNode.controlStructureType shouldBe ControlStructureTypes.WHILE

      val List(condition: Call, body) = whileNode.astChildren.l: @unchecked
    ...
    "have correct structure for defined? command" in {
      ...
      val List(callNode) = cpg.call.name("defined\\?").l
      callNode.code shouldBe "defined? x"
      callNode.lineNumber shouldBe Some(1)
      callNode.columnNumber shouldBe Some(0)

      val List(xCall) = cpg.call.name("x").l
      xCall.code shouldBe "x"
      xCall.lineNumber shouldBe Some(1)
      xCall.columnNumber shouldBe Some(9)
    }
    ...
    "have correct structure for if-else statement" in {
      ...
      val List(ifNode) = cpg.controlStructure.l
      ifNode.controlStructureType shouldBe ControlStructureTypes.IF
      ifNode.lineNumber shouldBe Some(2)

      val List(ifCondition, ifBlock, elseBlock) = ifNode.astChildren.l
      ifCondition.code shouldBe "x == 0"
      ifCondition.lineNumber shouldBe Some(2)

      val List(puts1) = ifBlock.astChildren.l
      puts1.code shouldBe "puts 1"
      puts1.lineNumber shouldBe Some(3)

      val List(puts2) = elseBlock.astChildren.l
      puts2.code shouldBe "puts 2"
      puts2.lineNumber shouldBe Some(5)
    }

    "have correct structure for implicit concatenation of non-interpolated \"\" string literals" in {
      val cpg = code("""
          |"x" "y" "z"   
          |""".stripMargin)

      val List(str: Literal) = cpg.literal.l: @unchecked

      str.code shouldBe """"x" "y" "z""""
      str.typeFullName shouldBe "__builtin.String"
      str.lineNumber shouldBe Some(2)
    }

    "have correct structure for an interpolated string literal" in {
      val cpg = code("""
          |"#{1 + 2} = 3"
          |""".stripMargin)

      val List(fmtStringCall) = cpg.call(Operators.formatString).l
      fmtStringCall.code shouldBe """"#{1 + 2} = 3""""
      fmtStringCall.lineNumber shouldBe Some(2)

      val List(fmtStringArg: Call) = fmtStringCall.argument.l: @unchecked
      fmtStringArg.name shouldBe Operators.formattedValue
      fmtStringArg.lineNumber shouldBe Some(2)

      val List(addition: Call) = fmtStringArg.argument.l: @unchecked
      addition.methodFullName shouldBe Operators.addition
      addition.code shouldBe "1 + 2"
      addition.argument.isLiteral.size shouldBe 2
    }

    "have correct structure for an implicit concatenation of interpolated string literals" in {
      val cpg = code("""
          |"#{3}" "= #{1+2}"
          |""".stripMargin)

      val List(fmtStringCall) = cpg.call(Operators.formatString).l
      fmtStringCall.code shouldBe """"#{3}" "= #{1+2}""""
      fmtStringCall.lineNumber shouldBe Some(2)

      val List(fmtFst: Call, fmtSnd: Call) = fmtStringCall.argument.l: @unchecked
      fmtFst.name shouldBe Operators.formattedValue
      fmtSnd.name shouldBe Operators.formattedValue

      fmtFst.code shouldBe "3"
      fmtSnd.code shouldBe "1+2"

      // TODO: should be more detailed
      fmtFst.argument.isLiteral.size shouldBe 1
      fmtSnd.argument.isCall.name(Operators.addition).size shouldBe 1
      fmtSnd.argument.isCall.name(Operators.addition).argument.isLiteral.size shouldBe 2
    }

    "have correct structure for empty top-level method declaration" in {
      val cpg = code("""
          |def bar
          |end
          |""".stripMargin)

      val List(barMethod) = cpg.method.nameExact("bar").l: @unchecked
      barMethod.fullName shouldBe "Test0.rb:<global>::program:bar"
      barMethod.lineNumber shouldBe Some(2)

      // TODO: should be more detailed
    }

    "have correct structure for empty top-level unary method declaration" in {
      val cpg = code("""
          |def foo(x)
          |end
          |""".stripMargin)

      val List(fooMethod) = cpg.method.nameExact("foo").l: @unchecked
      fooMethod.fullName shouldBe "Test0.rb:<global>::program:foo"
      fooMethod.lineNumber shouldBe Some(2)

      // TODO: should be more detailed
      val List(xParam) = fooMethod.parameter.l
      xParam.code shouldBe "x"
    }

    "have correct structure for class declaration with single method" in {
      val cpg = code("""
          |class C
          | def foo
          | end
      ...
      fooMethod.fullName shouldBe "Test0.rb:<global>::program.C:foo"
    }

    "have correct structure for a self.method declaration inside a class" in {
      val cpg = code("""
          |class C
          |  def self.foo
          |  end
    ...
    "have correct structure for an invocation without arguments and a 0-ary block" in {
      val cpg = code("""foo do
          | puts 1
          |end
          |""".stripMargin)

      val List(foo, puts) = cpg.call.l

      // TODO: should be more detailed and not rely on the order of cpg.call

      foo.name shouldBe "foo"
      puts.name shouldBe "puts"

      foo.argument.size shouldBe 0
      puts.argument.size shouldBe 1
    }

    // TODO(xavierp)
    //    "have correct structure for if-elsif-else statement" in {
    //      val cpg = code("""
    //          |if x == 0
    //          | 1
    //          |elsif x == 1
