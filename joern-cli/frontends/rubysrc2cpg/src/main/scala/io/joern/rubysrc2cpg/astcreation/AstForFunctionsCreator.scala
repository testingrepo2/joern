...
trait AstForFunctionsCreator(implicit withSchemaValidation: ValidationMode) { this: AstCreator =>
  ...
  protected def astForMethodDeclaration(node: MethodDeclaration): Ast = {
    ...
    val stmtBlockAst = ParserAst(node.body) match
      ...
      case _: StaticLiteralExpression =>
      ...
      case body =>
        logger.warn(
          s"Non-linear method bodies are not supported yet: ${body.text} (${body.getClass.getSimpleName}), skipping"
        )
        astForUnknown(body)
  ...
  protected def astForParameter(node: ParserNode, index: Int): Ast = {
    node match
      ...
      case node =>
        logger.warn(
          s"Non-mandatory parameters are not supported yet: ${node.text} (${node.getClass.getSimpleName}, skipping"
        )
        astForUnknown(node)
  }
